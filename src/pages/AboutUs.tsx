...
export default function AboutUs() {
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* ─── Hero ─────────────────────────────────────────── */}
      <section className="relative overflow-hidden bg-navy" style={{ minHeight: "520px" }}>
        {/* Background image */}
        <img
          src={aboutBannerImg}
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 py-20 grid grid-cols-1 lg:grid-cols-2 gap-12 items-end">
          <div className="animate-slide-up">
            <div className="section-label text-gold mb-4">About BluBox Exports</div>
            <h1 className="font-display text-4xl md:text-6xl font-semibold text-white leading-tight mb-6">
              Future-Ready Hardware Solutions Designed for Today's Business Needs
            </h1>
            <div className="w-16 h-0.5 bg-gold mb-6"></div>
            <p className="font-body text-white/70 text-lg leading-relaxed mb-8">
              Founded in 2012 in Chennai, Vasan Hardware began with a vision to craft reliable, high-quality door hardware, furniture fittings, and modular hardware solutions that seamlessly blend Indian craftsmanship with modern engineering.
            </p>
            <Link to="/contact" className="btn-gold text-sm inline-flex items-center gap-2">
              Partner With Us
              <ArrowRight size={14} />
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 animate-slide-up animate-slide-up-delay-2">
            {[
              ...
              <div key={i} className="border border-white/80 p-5 text-center" style={{ background: "rgba(200,169,106,0.08)" }}>
                <div className="font-display text-4xl font-bold text-white leading-none mb-2">{stat.value}</div>
                <div className="font-body uppercase tracking-widest text-[9px] font-bold text-white/60">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {COMMITMENTS.map((item, i) => (
              <div
                key={i}
                className={`relative overflow-hidden group animate-slide-up ${i % 2 === 0 ? "sm:border-r" : ""} ${i < 2 ? "border-b" : ""} border-border-subtle`}
                style={{ animationDelay: `${i * 80}ms`, minHeight: "280px" }}
              >
                {/* Background image */}
                <img
                  src={item.img}
                ...
                <div className="relative z-10 p-8 flex flex-col justify-end h-full" style={{ minHeight: "280px" }}>
                  <div className="w-10 h-10 bg-gold/20 border border-gold/40 flex items-center justify-center mb-4">
                    {item.icon}
                  </div>
                  <h3 className="font-body font-bold uppercase tracking-wide text-white text-sm mb-3">{item.title}</h3>
                ...
                {WHY_US.map((item, i) => (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <div className="font-display text-4xl font-bold text-gold/30 shrink-0 leading-none">{item.no}</div>
                      <div>
                        <div className="w-6 h-0.5 bg-gold mb-3"></div>
                        <h3 className="font-body font-bold uppercase tracking-widest text-navy text-sm mb-3">{item.title}</h3>
                        <ul className="space-y-2">
                          {item.points.map((pt, j) => (
                            <li key={j} className="flex items-start gap-2 text-foreground/65 font-body text-sm leading-relaxed">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2">
            ...
            <div className="px-8 md:px-12 py-16">
              ...
              <h2 className="font-display text-4xl font-semibold text-navy mb-3">A Decade of Excellence</h2>
              <div className="w-14 h-0.5 bg-gold mb-10"></div>
              <div className="space-y-0 border-l-2 border-border-subtle ml-4">
                {[
                  { year: "2012", label: "Founded in Chennai as Vasan Hardware" },
                  { year: "2015", label: "Expanded to B2B supply for builders & contractors" },
                  { year: "2019", label: "Launched BluBox Exports, our global arm" },
                  { year: "2024", label: "50+ export destinations worldwide" },
                ].map((item, i) => (
                  <div key={i} className="relative pl-8 pb-8 animate-slide-up" style={{ animationDelay: `${i * 80}ms` }}>
                    <div className="absolute -left-[9px] top-0 w-4 h-4 bg-gold border-2 border-white"></div>
                    <div className="font-display text-2xl font-bold text-gold mb-1">{item.year}</div>
                    <div className="font-body text-foreground/65 text-sm leading-relaxed">{item.label}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="relative overflow-hidden" style={{ minHeight: "450px" }}>
              ...
              <div className="absolute bottom-0 left-0 right-0 px-8 py-6">
                <div className="font-body uppercase tracking-[0.2em] text-gold text-[10px] font-bold mb-1">Exporting to 50+ countries</div>
                <div className="font-display text-2xl text-white font-semibold">Trusted Globally Since 2012</div>
              </div>
            </div>
          </div>
        </div>
